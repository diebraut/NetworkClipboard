    ...
    : QObject(parent)
{
    ...
    if (!m_discoverySocket.bind(QHostAddress::AnyIPv4,
        ...
        qWarning() << "NetworkClipboardAndroid: could not listen for UDP presence events:"
...
void NetworkClipboardClient::pollLatest()
{
    ...
    QNetworkReply *reply = m_network.get(latestRequest);
    QTimer::singleShot(90000, reply, [reply]() {
        if (reply->isRunning())
            reply->abort();
    });
...
bool NetworkClipboardClient::sendDiscoveryDatagrams()
{
    if (m_discoverySocket.state() == QAbstractSocket::UnconnectedState
        && !m_discoverySocket.bind(QHostAddress::AnyIPv4,
                                   DiscoveryPort,
                                   QUdpSocket::ShareAddress | QUdpSocket::ReuseAddressHint)) {
        qWarning() << "NetworkClipboardAndroid: could not bind UDP discovery socket:"
                   << m_discoverySocket.errorString();
        return false;
    }
